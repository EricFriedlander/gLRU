%  Generates figures 3 and 5 and data from Table 2
...
% out1 = zeros(2, 2, 3, 2, 4, 3, 4,100,10);
mypool = parpool(4)
parfor g = 1:4
    for a = 1:2
        for b = 1:2
...
fileLen = [60*10 60*30 60*60];
% out2 = zeros(2, 2, 3, 2, 4, 3, 4,100,11);
out2 = zeros(2, 2, 3, 2, 4, 3, 4,10);
mypool = parpool(4)
parfor g = 1:4
    for a = 1:2
        for b = 1:2
            for c = 1:3
                for d = 1:2
                    for e = 1:4
                        for f = 1:3     
%                             parfor h = 1:100
%                                out2(a,b,c,d,e,f,g,h,:) = ...
%                                    CachesimCompEff(n,alpha(a),10,Cprop(b),sizeType(c), ...
%                                    stupd(d), clen(e), traffic(f), procrate(g),.01);
                              out2(a,b,c,d,e,f,g,:) = ...
                                   CachesimCompEff(n,alpha(a),fileLen(c),Cprop(b),4, ...
                                   stupd(d), clen(e), traffic(f), procrate(g),.01);
%                             end
                        end
                        f
...
save('simout2.mat','out2')

% tic
% CachesimCompEff(1000,.8,10,.1,1, ...
%                                    4, 1, .9,30,.01)
% toc
...
%% Examine Different Popularities
